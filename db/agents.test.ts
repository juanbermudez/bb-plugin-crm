...
describe("CRM agent workspace persistence", () => {
  it("keeps the migration v5 agent graph intact after later migrations", async () => {
    ...
    try {
      expect(db.prepare("SELECT value FROM crm_metadata WHERE key = 'schema_version'").pluck().get()).toBe("8");
      expect(db.prepare("SELECT MAX(id) FROM _bb_migrations").pluck().get()).toBe(7);
