...
| Compact list controls | One non-wrapping table toolbar: full-placeholder search and searchable vertical filters left; icon-only sort, saved views, columns, and archive right; locally ejected BB-themed beUI Select and Combobox-derived menus; one icon size/style with top tooltips; no visible `All records` control or redundant result count | done | 2/5 |
